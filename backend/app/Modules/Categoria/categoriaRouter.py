...
def crear_categoria(
    ...
    try:
        return servicio_categoria.crear(unidad_trabajo, datos_categoria)
...
def listar_categorias(
    ...
    return servicio_categoria.listar(unidad_trabajo, offset=offset, limite=limite, nombre=nombre, activo=activo)
...
def obtener_categoria(
    ...
    try:
        return servicio_categoria.obtener_por_id(unidad_trabajo, id_categoria)
    except RecursoNoEncontradoError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    except ConflictoDeNegocioError as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error)) from error
...
def actualizar_categoria(
    ...
    try:
        return servicio_categoria.actualizar(unidad_trabajo, id_categoria, datos_categoria)
    except RecursoNoEncontradoError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    except ConflictoDeNegocioError as error:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=str(error)) from error
...
def eliminar_categoria(
    ...
    try:
        servicio_categoria.eliminar(unidad_trabajo, id_categoria)
